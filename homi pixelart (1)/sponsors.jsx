// === Sponsor logos ===
// High-resolution SVG brand marks. These render as crisp vector at any zoom
// level. Each logo is a 64x64 viewBox component designed to look like a real
// startup/sponsor brand mark.
//
// HACKATHON SPONSORS: replace any of these with the partner's actual SVG (or
// use an <img> with their PNG). The slight pixel feel comes from
// shape-rendering="crispEdges" + image-rendering: pixelated on the wrapper.

function LogoHalo({ size = 64 }) {
  return (
...
function LogoComet({ size = 64 }) {
  const gradId = 'cometGrad-' + Math.floor(Math.random() * 100000);
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" style={{ display: 'block' }}>
      <rect width="64" height="64" fill="#1a1326" />
      <defs>
...
const SPONSOR_KEYS_HD = Object.keys(SPONSORS);

// SponsorLogo: renders a logo at a specified CSS size.
function SponsorLogo({ keyName, size = 24, pixelated = true, style }) {
...
Object.assign(window, { SPONSORS, SPONSOR_KEYS_HD, SponsorLogo });
